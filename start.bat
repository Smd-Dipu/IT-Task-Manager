...
if "%FORCE%"=="1" goto :install_backend
echo  [2/5] Backend dependencies already installed.  ^(skip^)
goto :backend_done
:install_backend
...
if "%FORCE%"=="1" goto :install_frontend
echo  [3/5] Frontend dependencies already installed.  ^(skip^)
goto :frontend_done
:install_frontend
...
echo  [4/5] Frontend build already present.  ^(skip^)
goto :build_done
:build_frontend
...
REM ---- 5. Ensure config + data directories ----
echo  [5/5] Ensuring configuration and data directories...
...
echo  Starting TaskFlow on http://localhost:%PORT%
